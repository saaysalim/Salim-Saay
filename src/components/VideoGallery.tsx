...
export function VideoGallery() {
  ...
  // Deduplicate: keep first occurrence (keyed by youtubeId when present, otherwise title)
  const uniqueVideos: VideoItem[] = Array.from(
    videos.reduce((map, v) => {
      const key = (v.youtubeId ?? v.title).toString();
      if (!map.has(key)) map.set(key, v);
      return map;
    }, new Map<string, VideoItem>()).values()
  );

  // Ensure currentVideo is valid if the list changes
  useEffect(() => {
    if (currentVideo >= uniqueVideos.length) {
      setCurrentVideo(0);
    }
  }, [uniqueVideos.length, currentVideo]);
  ...
  const current = uniqueVideos[currentVideo] || uniqueVideos[0];
    ...
    <section id="video-gallery" className="py-20 px-4 bg-background min-h-screen">
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="flex justify-center space-x-4 mb-8">
          <Button variant="outline" onClick={handlePrevious} className="px-8">Previous</Button>
        ...
        <div className="grid gap-4">
          ...
          {uniqueVideos.map((video, index) => (
            <div
              key={index}
              className={`p-4 border rounded-lg cursor-pointer transition-colors ${index === currentVideo ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50'}`}
              onClick={() => setCurrentVideo(index)}
              role="button"
              tabIndex={0}
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setCurrentVideo(index); }}
            >
              <div className="flex items-center justify-between">
                <div>
